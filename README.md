...
| `variant` | `"bar"`(기본, 머리말 **위** 띠) · `"inline"`(머리말 **안** 한 줄) — 3절 |
...
이고, `"inline"`(머리말 **안** 한 줄)이 선택지다. 이 묶음은 「어디에 앉았는지」를
스스로 알아내려 하지 않는다 — 머리말 구조는 사이트마다 다르고, 짐작하는 순간
틀리기 시작한다.

| | `variant="bar"` (기본) | `variant="inline"` |
|---|---|---|
| 앉는 자리 | 머리말 **위**, 독립된 띠 | 머리말 **안**, 제목과 같은 줄 |
| 바탕 | 머리말보다 한 단 눌린 회색 + 아래 테두리 | 없다 — 머리말 위에 그대로 얹힌다 |
| 지금 있는 서비스 | 머리말과 같은 색 칸으로 **떠 있다**(브라우저 탭 은유) | **굵기 + 진한 글자색 + 2px 밑줄** |
| 폰(<768px) | 이름까지 다 보이고 띠 안에서 굴러간다 | **아이콘만** 보인다(이름은 낭독기에만 남는다) |
| 사이트 머리말 | 한 줄도 고치지 않는다 | 머리말이 이 조각을 제 줄 안에 그려야 한다 |

### 왜 둘인가 — 한때는 "위"가 유일한 답이었다

처음에는 **위에만** 앉혔다. 사이트 다섯의 머리말이 제각각이라(A/S 는 햄버거 +
제목 + 알림종이 한 줄에 든 `flex`, 계측기는 항목이 줄바꿈되는 `flex-wrap`,
개선요청은 좌우 두 덩이) 칸이 다섯~열 개 붙는 목록을 끼워 넣으면 **사이트마다
다른 방식으로 깨진다**고 본 것이다. 그 판단은 지금도 맞다 — 그래서 기본값은
바뀌지 않았다.

바뀐 것은 A/S 다. 회색 띠가 한 층을 더 차지해 **화면 위쪽이 답답하고 창이
그만큼 작아진다**고 사용자가 지적했고(2026-09-18), 그 머리말은 넓은 화면에서
제목과 알림종 사이가 통째로 비어 있었다. 거기에 들어가면 세로 한 줄이 돌아온다.
그래서 A/S 만 `"inline"` 을 고른다.

🔴 **새로 붙이는 사이트가 기본으로 고를 것은 여전히 `"bar"` 다.** `"inline"` 은
그 머리말이 (1) 한 줄짜리 `flex` 이고, (2) 가운데가 비어 있고, (3) 그 자리를
**줄어들 수 있게**(`min-w-0`) 내줄 수 있을 때에만 값이 있다. 셋 중 하나라도
아니면 위에 앉히는 편이 싸다.
...
| njlee (계측기) | 머리말 위(기본). `src/app/(internal)/layout.tsx` 의 `<AppHeader …/>` 바로 위 | 같은 layout 이 서버에서 |
| dss-improvements (개선요청) | 머리말 위(기본). `src/app/(app)/layout.tsx` 의 `<AppHeader …/>` 바로 위 | 같은 layout 이 서버에서 |
...
- **놓이는 방식이 모습마다 다르다.** `"bar"` 는 세로 `flex` 안에서 눌리지 않게
  `className="shrink-0"`. `"inline"` 은 반대로 **줄어들 수 있어야** 한다 —
  가로 `flex` 안에서 `min-w-0 flex-1` 로 두면 남는 자리만 쓰고, 목록이 길어도
  자기 안에서 굴러간다(옆의 아이콘 버튼들을 밀어내지 않는다).
...
- 🔴 **기본 모습이 예전 그대로다** — `variant` 를 안 넘긴 것과 `"bar"` 가 글자
  하나까지 같고, 새 모습은 클래스 하나만 더한다(나머지 차이는 전부 CSS 에 있고,
  그 규칙은 하나도 빠짐없이 `.dss-menu--inline` 안에만 걸린다)
- 🔴 새 모습이 **바탕과 아래 테두리를 갖지 않는다**, 두 모습 모두 지금 있는
  서비스를 알아볼 수 있게 표시한다, 빈 목록이면 둘 다 아무것도 그리지 않는다
- 🔴 폰에서 이름을 **눈에서만** 감춘다(낭독기는 그대로 읽는다)는 것과, 아이콘이
  없는 칸이 빈 칸이 되지 않는다(이름 첫 글자로 메운다)는 것
